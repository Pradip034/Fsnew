...
// Ensure fetch is correctly imported from node-fetch
let fetch;
try {
    const nodeFetch = require('node-fetch');
    // node-fetch v3+ exports the function directly as default.
    // node-fetch v2 also exports it directly.
    // This check helps ensure 'fetch' is the function, even if 'node-fetch' module structure varies slightly.
    fetch = typeof nodeFetch === 'function' ? nodeFetch : nodeFetch.default;
    if (typeof fetch !== 'function') {
        throw new Error("node-fetch did not export a function correctly. Please check your node-fetch version and Node.js environment.");
    }
} catch (error) {
    console.error("Error importing node-fetch:", error.message);
    console.error("Please ensure you have installed node-fetch: `npm install node-fetch`");
    process.exit(1); // Exit the process if fetch isn't available
}
...
const API_KEY = "AIzaSyApgx6euvrYcXr0e_ZZTKWUx3ogCfWC2bE"; // Leave this empty for gemini-2.0-flash in Canvas environment
